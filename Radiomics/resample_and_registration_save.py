...
resampled_path='/home/ccids-sw/Desktop/test_resampled'
save_path='/home/ccids-sw/Desktop/test_registered'
...
# subjects=os.listdir(resampled_path)
...
for sub in subjects:
    ...
    print('\nsubject -- {} start\n'.format(sub))
    # whole_imgs=os.listdir(os.path.join(resampled_path))
    whole_imgs=[img for img in os.listdir(resampled_path) if sub in img]

    imgs=sorted([os.path.join(resampled_path, a) for a in whole_imgs if 'mask' not in a and 'pbm' not in a]) # T1_GD, T2
    mask=sorted([os.path.join(resampled_path, a) for a in whole_imgs if 'mask' in a]) # ce, necro, t2 ..

    # imgs = sorted([os.path.join(resampled_path, sub, a) for a in whole_imgs if 'mask' not in a and 'pbm' not in a])  # T1_GD, T2
    # mask = sorted([os.path.join(resampled_path, sub, a) for a in whole_imgs if 'mask' in a])  # ce, necro, t2 ..
